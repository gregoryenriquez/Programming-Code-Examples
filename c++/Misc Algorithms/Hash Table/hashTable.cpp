...
void	HashTable<DataType>::insert(DataType d, string s)
{
	...
	int orig_key = key;
	if (key < 0) key = -key;
	if (orig_key < 0) orig_key = -orig_key;
	int tries = 100;
	int quad = 1;
	...
	else
	{
		while (!current->isEmpty() && tries > 0)
		{
			...
			key = (orig_key + quad*quad) % table_size;
			/*
			...
			*/
			...
			if (current && current->isEmpty())
			{
				...
				current->setNotEmpty();
				totalEntries++;
				cout << "original key: " << orig_key << endl;
...
template <class DataType>
int		HashTable<DataType>::hashf(string keyString)
{
	//cout << "Data: " << d << endl;
